...
(function (path) {
  ...
  $('.btn-add').click(function (e) {
    ...
    $modal.find('form').attr('action', path + '/contact?cmd=post'); // 새로입력
  });
  ...
  // 테이블에서 수정버튼 클릭시 => 모달창(id로 그 연락처 내용을 채움)
  // $('table').on('이벤트', '셀렉터(selector)', function (e){}) => 클래스가 btn-edit 버튼일때만 이 함수 실행
  $('table').on('click', '.btn-edit', function (e) {
    const $modal = $('#modal-add-update');
    // 모달안에 title-add-upd를 찾음
    $modal.find('#title-add-upd').text('업데이트');
    $modal.find('form').attr('action', path + '/contact?cmd=update'); // 실제 수정하기

    $.ajax({
      type: 'POST',
      url: path + '/contact?cmd=edit', // 수정하기 창
